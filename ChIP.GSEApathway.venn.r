...
##################################read files
saminfo=read.csv("./data/3_BE2//sample.combined.chip.csv")
anno=read.table("./data/3_BE2//RARA_SMAD4_combined.bed.anno",header = T)
d=read.table("./data/3_BE2//all.counts.data",header = T)
...
for (i in seq_along(tb)){
   # if(i!=2){
   # d3=as.matrix(add_anno[,frag[[i]]])
   #  mode(d3) <- "integer"
   #  samra=saminfo[frag[[i]],]
   #  samra$type="RA"
   #  samra[1:2,]$type="DMSO"
...
for (i in seq_along(levels(saminfo$source))){
  ...
  y <- GSEA(genelist, TERM2GENE = bplan,pvalueCutoff = 0.05)
