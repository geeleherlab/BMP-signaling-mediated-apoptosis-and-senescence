...
##################################read files, here use files from BE2 cell line as an example, 

saminfo=read.csv("./data/3_ChIP_GSEApathway_venn/3_BE2/sample.combined.chip.csv")
anno=read.table("./data/3_ChIP_GSEApathway_venn/3_BE2/RARA_SMAD4_combined.bed.anno",header = T)
d=read.table("./data/3_ChIP_GSEApathway_venn/3_BE2/all.counts.data",header = T)
...
add_anno=inner_join(df,anno,by= "Region")[,-c(10:16)]
...
for (i in seq_along(levels(saminfo$source))){
  ...
  y <- GSEA(genelist, TERM2GENE = bplan,pvalueCutoff = 0.1)###ajust pvalue here if no result available
